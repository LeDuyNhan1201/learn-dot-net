...
public class Version2DocumentTransfomer
    (IOptions<ApiDocsOptions> apiDocsOptions) : IOpenApiDocumentTransformer
{
    ...
    public Task TransformAsync(OpenApiDocument document, OpenApiDocumentTransformerContext context, CancellationToken cancellationToken)
    {
        ...
        document.Paths
            .Where(path =>
                path.Key.StartsWith($"/{Version2}", StringComparison.OrdinalIgnoreCase))
            .ToList()
            .ForEach(path => filteredPaths.Add(path.Key, path.Value));

        document.Paths = filteredPaths;

        Console.WriteLine($"Filtered OpenAPI document paths for version '{Version2}':");
